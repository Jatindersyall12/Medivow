...
public class PaymentFharmacyActivity extends BaseActivity {
    private EditText et_amount;
    TextView tv_total_amount,tv_name;
    String clientId,hospitalName;
    Button btnProcess;
    String amount;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_payment_fharmacy);
        clientId=getIntent().getStringExtra("clientId");
        hospitalName=getIntent().getStringExtra("hospitalName");
        setUpToolBarProfile("Pharmacy Fee",true);

        init();

        tv_name.setText(hospitalName);
        tv_total_amount.setText(AppPreferences.getPreferenceInstance(PaymentFharmacyActivity.this).getPayment());

        btnProcess.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!et_amount.getText().toString().equals("")){
                    int walletAmount=Integer.valueOf(AppPreferences.getPreferenceInstance(PaymentFharmacyActivity.this).getPayment());
                    int Amount=Integer.valueOf(et_amount.getText().toString());
                    if (Amount<walletAmount){
                        amount=et_amount.getText().toString();
                        makePayment(AppPreferences.getPreferenceInstance(PaymentFharmacyActivity.this).getUserId(),
                                clientId, "0",et_amount.getText().toString(),"0","4");
                    }else {
    ...
    public void showPaymentAlertDialog(){
        final BottomSheetDialog dialog = new BottomSheetDialog(PaymentFharmacyActivity.this, R.style.CustomDialogTheme);
        ...
        callButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(PaymentFharmacyActivity.this, WalletActivity.class);
    ...
    public void showSuccessDialog(){
        ...
        callButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(PaymentFharmacyActivity.this, HomeActivity.class);
                startActivity(intent);
                dialog.dismiss();
            }
        });
